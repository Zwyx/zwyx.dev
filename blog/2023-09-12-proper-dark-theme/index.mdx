...
Does implementing dark theme manually sound like reinventing the wheel?

:::tip
The problem with dark theme libraries is that they depend on a specific stack.

And the problem with UI/component libraries is that they often — _incredibly often_, based on the dozen I've tested — don't correctly implement the following requirements.
:::
...
- allow the user to change their theme preference,
- store it in local storage,
- listen for changes of the system's theme,
- update the HTML root element's class according to the user preference and the system's theme.

The code is a bit long so I won't include it here. Instead, I invite you to have a look at the [implementation made with TypeScript and React](https://github.com/Zwyx/proper-dark-theme/blob/main/src/lib/ThemeContext.tsx) for the demo project.

## Note on the three states

I consider this three-state implementation to be the best at the moment, but I hope that it won't be necessary in the future. Indeed, when all operating systems and all web browsers will implement dark theme seemlessly, then offering to the user the possibility of changing the theme for a particular website might start to be seen as unnecessary. Or at least, the setting could be buried in a dialog box instead of being directly available on the top right of the page.
...
- the app cannot know which theme is currently displayed when the user preference is to follow the system's theme (Tailwind's `dark:` selector, for instance, doesn't work).
