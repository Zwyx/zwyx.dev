...
In order to add the Giscus component to our blog posts, we are going to use Docusaurus' swizzle method to wrap one component of the template (read about swizzling [here](https://docusaurus.io/docs/next/swizzling)):

```bash
yarn run swizzle @docusaurus/theme-classic BlogPostPaginator --wrap
```

`BlogPostPaginator` is the component showing "Newer Post" and "Older Post" links at the bottom of a blog post page.

The previous command created a new file:
...
This file is a wrapper for the `BlogPostPaginator` component. We can very easily add our new `GiscusComments` component just above it:
...
:::tip
I didn't use the `BlogPostItem` component, because it is displayed for every blog posts on the main blog listing page. It would be possible to display the Giscus component conditionnally though (as see [here](https://dev.to/m19v/how-to-add-giscus-comments-to-docusaurus-439h)).
:::
...
The site is deployed on very push to the `master` branch. The [code](https://github.com/Zwyx/zwyx.github.io/blob/master/.github/workflows/deploy-to-github-pages.yml) should be self-explanatory with enough knowledge on [GitHub Actions](https://docs.github.com/en/actions) and [GitHub Pages](https://docs.github.com/en/pages).
